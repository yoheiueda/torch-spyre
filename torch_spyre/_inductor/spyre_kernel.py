    ...
    def store(
        ...
        if isinstance(value, UnimplementedOp):
            ...
            if all(e == 0 for e in in_coords) and not all(e == 0 for e in out_coords):
                ...
                # buffer; the iter-axis reorder that follows can collapse
                # the input/output free_symbol distinction even though the
                # op is still semantically a restickify.  The pass announces
                # intent via this attribute so the RESTICKIFY-gated
                # ceil-div + backGap padded-read path stays armed.
                op = RESTICKIFY_OP
            else:
                op = IDENTITY_OP
