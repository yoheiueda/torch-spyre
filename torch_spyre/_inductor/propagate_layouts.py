...
def _single_arg_op_layout(
    ...
    def compute_dim_order(stick_dim):
        dim_order = [
            d for d in range(len(c_size)) if d != stick_dim and out_coords[d] != 0
        ]
        dim_order += [
            d for d in range(len(c_size)) if d != stick_dim and out_coords[d] == 0
        ]
...
def _clone_layout(
    ...
    """
    Compute the output STL(s) for a clone op given one candidate input STL.
    Called once per candidate input STL to produce corresponding output STL(s).
    """
    data = op.data
    ...
    op.restick_cost_fn = FixedInOutNode.from_args(args, out_stl, [layouts[0]])
    return [out_stl]
